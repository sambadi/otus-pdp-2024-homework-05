...
class InMemoryStore(Store):
    ...
    def cache_set(self, key: str, value: Any, ttl: int = 60):
        self.__storage[key] = json.dumps(value).encode("utf-8")
        ...
        def wrapper(*args):
            ...
            for c in cases:
                try:
                    ...
                    raise ValueError(
                        f"Test '{f.__name__}'[{i}] with args '{c}' is failed to run. Error: {e}"
                    ) from e
                i += 1
...
class TestSuite(unittest.TestCase):
    ...
    def preheat_kv_store(self, ids: list):
        interests = [
            "cars",
            "pets",
            "travel",
        ...
        for cid in ids:
            self.store.cache_set(f"i:{cid}", random.sample(interests, 2))
    ...
    def test_ok_interests_request(self, arguments):
        ...
        self.preheat_kv_store(arguments["client_ids"])
        self.set_valid_auth(request)
        response, code = self.get_response(request)
        self.assertEqual(api.OK, code, arguments)
